...
( function( $ ) {
	...
	( function() {
		...
		menuToggle.on( 'click.esenia', function() {
	...
	( function() {
		...
		function toggleFocusClassTouchScreen() {
			if ( window.innerWidth >= 910 ) {
				$( document.body ).on( 'touchstart.esenia', function( e ) {
					if ( ! $( e.target ).closest( '.main-navigation li' ).length ) {
						$( '.main-navigation li' ).removeClass( 'focus' );
					}
				} );
				siteNavigation.find( '.menu-item-has-children > a' ).on( 'touchstart.esenia', function( e ) {
				...
				siteNavigation.find( '.menu-item-has-children > a' ).unbind( 'touchstart.esenia' );
			}
		}

		if ( 'ontouchstart' in window ) {
			$( window ).on( 'resize.esenia', toggleFocusClassTouchScreen );
			toggleFocusClassTouchScreen();
		}

		siteNavigation.find( 'a' ).on( 'focus.esenia blur.esenia', function() {
	...
	$( document ).ready( function() {
		...
		$('.posts-slider').each( function(){
			...
			slider.flexslider( {
				...
				init: function( slider ) {
					...
					slider.doMath();
					$(window).on('resize.esenia', function() {
						sliderWidthInit(slider);
						slider.doMath();
					});
				}
		...
		$('.search-toggle').on( 'click.esenia', function() {
		...
		$('.site-search .search-field, .site-search .search-submit').on( 'focus', function() {
 		...
 		$('.load-more a').on('click.esenia', function (e) {
			e.preventDefault();
